...
class AirportFlightGraph:
    ...
    def find_path_between_multiple_cities(self, cities):
        """Finds an optimized flight path through multiple cities using airport connections efficiently."""
        if len(cities) < 2:
        ...
        # **Find best airport connections between cities**
        total_path = []
        total_distance = 0
        ...
        for i in range(len(cities) - 1):
            ...
            # **Instead of looping over all pairs, pick the best airport for each city pair**
            for src in src_airports:
                for dst in dst_airports:
    ...
    def _get_shortest_airport_path(self, src, dst): # shortest path function
        """Returns the shortest path and distance between two airports."""
        try:
